...
static void BM_AddOrders(benchmark::State& state) {
    for (auto _ : state) {
        ...
        for (int i = 0; i < state.range(0); ++i) {
            book.addOrder(id, Side::Buy,  100 - (i % 10), 10, id); ++id;
            book.addOrder(id, Side::Sell, 110 + (i % 10), 10, id); ++id;
        }
...
static void BM_MatchSweep(benchmark::State& state) {
    ...
    for (auto _ : state) {
        ...
        for (int i = 0; i < levels; ++i) {
            book.addOrder(id, Side::Sell, 100 + i, 10, id); ++id;
        }
        state.ResumeTiming();

        // Aggressor buy that crosses all resting asks
        book.addOrder(id, Side::Buy, 100 + levels, levels * 10, id); ++id;
        benchmark::DoNotOptimize(book);
    }
}
